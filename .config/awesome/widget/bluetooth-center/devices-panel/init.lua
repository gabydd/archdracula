...
awesome.connect_signal("bluetooth::devices:refreshPanel", function()
	resetDevicePanelLayout()
	awful.spawn.with_line_callback([[bash -c "bluetoothctl devices"]], {
		stdout = function(line)
			awful.spawn.easy_async_with_shell(
				"bluetoothctl info "
					.. line:match("[%w:]+", 8)
					.. " | grep 'Connected' | sed 's/Connected: //' | sed 's/\t//g'",
				function(stdout)
					bluetoothDeviceAdd({
