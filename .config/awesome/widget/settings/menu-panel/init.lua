		...
		if i == category then
			...
			for j, label in pairs(pages[i]) do
				if j == subcategory then
					subCategoriesPanelLayout:insert(
						#subCategoriesPanelLayout.children + 1,
						box.create(i, j, true, true)
					)
				else
					subCategoriesPanelLayout:insert(
						#subCategoriesPanelLayout.children + 1,
						box.create(i, j, false, true)
					)
				end
			end
		else
...
awesome.connect_signal("settings::categories:setPanel", function(category, subcategory)
