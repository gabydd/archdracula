...
elements.create = function(dateString, int, elementTable)
	...
	toDoWidget:connect_signal("mouse::enter", function()
		toDoBg.bg = colors.color3
		toDoIcon.image = icons.toDoDone
	end)

	toDoWidget:connect_signal("mouse::leave", function()
		toDoBg.bg = colors.color2
		toDoIcon.image = icons.toDo
	end)

	-- Remove this box from panel if clicked
	toDoWidget:buttons(gears.table.join(awful.button({}, 1, nil, function()
